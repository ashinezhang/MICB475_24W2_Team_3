...
ggsave(filename = "data/Aim3/shannon_stats.png"
       ...
       , height=5, width=6)
#Shannon facet
plot_shannon_facet <- ggplot(sample_data(colombia_rare), aes(sex,shannon_values$Shannon)) + xlab("") + ylab("Shannon Index") + geom_boxplot() + 
  stat_compare_means(comparisons = list(c("female", "male")), method = "wilcox.test",label = "p.signif", p.adjust.method = "BH", paired=FALSE)+
  scale_x_discrete(labels = c("female" = "Female", "male" = "Male"))+
  theme(axis.title.y=element_text(size=15), axis.text.x = element_text(size=15, angle = 90, vjust = 0.5),  strip.text = element_text(size = 15))+
 facet_wrap(~ diabetic_status)
plot_shannon
ggsave(filename = "data/Aim3/shannon_stats_facet.png"
       , plot_shannon_facet
       , height=5, width=6)
...
ggsave(filename = "data/Aim3/chao1_stats.png"
       ...
       , height=6, width=6)
#Chao1 facet
plot_chao1_facet <- ggplot(sample_data(colombia_rare), aes(sex,chao1_values$Chao1)) + xlab("") + ylab("Chao1 Index") + geom_boxplot() + 
  stat_compare_means(comparisons = list(c("female", "male")), method = "wilcox.test",label = "p.signif", p.adjust.method = "BH", paired=FALSE)+
  theme(axis.title.y=element_text(size=15), axis.text.x = element_text(angle = 90, vjust = 0.5, size=15), strip.text = element_text(size = 15))+
  scale_x_discrete(labels = c("female" = "Female", "male" = "Male"))+
  facet_wrap(~ diabetic_status)+
  ylim(NA, 360)
plot_chao1_facet

ggsave(filename = "data/Aim3/chao1_stats_facet.png"
       , plot_chao1_facet
       , height=6, width=6)
...
plot_pd <- ggplot(sample_data(colombia_rare), aes(diabetic_status_and_sex, phylo_dist$PD)) + xlab("") + ylab("Phylogenetic Diversity") + geom_boxplot() + 
  stat_compare_means(comparisons = list(c("Diabetic_female","Diabetic_male"), c("Non-diabetic_female","Non-diabetic_male"), c("Pre-diabetic_female","Pre-diabetic_male")), method = "wilcox.test",label = "p.signif", p.adjust.method = "BH", paired=FALSE)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))+
  scale_x_discrete(labels = function(x) gsub("_", " ", x))+
  ylim(NA, 25)
...
#PD_facet
plot_pd_facet <- ggplot(sample_data(colombia_rare), aes(sex, phylo_dist$PD)) + xlab("") + ylab("Phylogenetic Diversity") + geom_boxplot() + 
  stat_compare_means(comparisons = list(c("female", "male")), method = "wilcox.test",label = "p.signif", p.adjust.method = "BH", paired=FALSE)+
  theme(axis.title.y=element_text(size=15), axis.text.x = element_text(angle = 90, vjust = 0.5, size=15), strip.text = element_text(size = 15))+
  scale_x_discrete(labels = c("female" = "Female", "male" = "Male"))+
  facet_wrap(~ diabetic_status)+
...
plot_pd_facet
ggsave(filename = "data/Aim3/plot_PD_stats_facet.png", plot_pd_facet, height=4, width=6)
