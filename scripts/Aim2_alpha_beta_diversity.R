...
wilcox_shannon<-pairwise.wilcox.test(shannon_data$`shannon_values$Shannon`, shannon_data$diabetic_status,
                                     p.adjust.method = "BH")
print(wilcox_shannon)


plot_shannon <- ggplot(sample_data(colombia_rare), aes(diabetic_status,shannon_values$Shannon)) + xlab("") + ylab("Shannon Index") + geom_boxplot() + 
  stat_compare_means(comparisons = list(c("Diabetic","Non-diabetic"), c("Diabetic","Pre-diabetic"), c("Non-diabetic","Pre-diabetic")), method = "wilcox.test",label = "p.signif", p.adjust.method = "BH")+
...
wilcox_chao1<-pairwise.wilcox.test(chao1_data$`chao1_values$Chao1`, chao1_data$diabetic_status,
                                   p.adjust.method = "BH")
print(wilcox_chao1)
#Shannon plot with stats
plot_chao1 <- ggplot(sample_data(colombia_rare), aes(diabetic_status,chao1_values$Chao1)) + xlab("") + ylab("Chao1 Index") + geom_boxplot() + 
  stat_compare_means(comparisons = list(c("Diabetic","Non-diabetic"), c("Diabetic","Pre-diabetic"), c("Non-diabetic","Pre-diabetic")), method = "wilcox.test",label = "p.signif", p.adjust.method = "BH")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))+
  scale_x_discrete(labels = function(x) gsub("_", " ", x))+
  ylim(NA, 450)
...
wilcox_results<-pairwise.wilcox.test(faith_pd_data$`phylo_dist$PD`, faith_pd_data$diabetic_status,
                                     p.adjust.method = "BH")
print(wilcox_results)
# plot any metadata category against the PD
plot_pd <- ggplot(sample_data(colombia_rare), aes(diabetic_status, phylo_dist$PD)) + xlab("") + ylab("Phylogenetic Diversity") + geom_boxplot() + 
  stat_compare_means(comparisons = list(c("Diabetic","Non-diabetic"), c("Diabetic","Pre-diabetic"), c("Non-diabetic","Pre-diabetic")), method = "wilcox.test",label = "p.signif", p.adjust.method = "BH")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))+
  scale_x_discrete(labels = function(x) gsub("_", " ", x))+
  ylim(NA, 25)
